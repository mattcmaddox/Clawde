...
- **What:** for each configured free upstream with ≥1 key, probe every key
  ...
  `query_rate_limits()` helpers (HEAD/GET to `/v1/models`, 5s timeout,
  **zero tokens spent** — the "low-token ping" idea, better than a token
  probe).
