...
impl SlashCommand for ManagedAgentsCommand {
    ...
    fn arg_completions(&self, _partial: &str) -> Vec<ArgCompletion> {
        ...
        static COMPLETIONS: OnceLock<Vec<ArgCompletion>> = OnceLock::new();
        COMPLETIONS
            .get_or_init(|| {
                let mut out = vec![
                    ArgCompletion {
                ...
                for preset in clawde_core::builtin_managed_agent_presets() {
                    out.push(ArgCompletion {
                        value: format!("preset {}", preset.name),
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        use clawde_core::{builtin_managed_agent_presets, BudgetSplitPolicy, ManagedAgentConfig};
