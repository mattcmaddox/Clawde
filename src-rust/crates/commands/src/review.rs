...
impl SlashCommand for ReviewCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let diff = if base.is_empty() {
            ...
            if staged.is_empty() {
                clawde_core::git_utils::get_unstaged_diff(&repo_root)
            } else {
                staged
            }
        } else {
            ...
            if let Some(pr_num) = pr_number {
                ...
                if let Some((owner, repo)) = detect_github_owner_repo(&repo_root) {
                    let comment_body = format!(
                        "## Clawde Code Review\n\n{}\n\n---\n*Generated by [Clawde](https://claude.ai/claude-code)*",
                        review_text
                    );
                    ...
                    // Shared client carries the required User-Agent, Accept,
                    // and X-GitHub-Api-Version headers; add the bearer token.
                    let http = clawde_core::github::api_client();
