...
use clawde_core::oauth_config::CodexTokens;
use clawde_tui::DeviceAuthEvent;
...
pub fn build_auth_url(code_challenge: &str, state: &str) -> String {
    format!(
        "{}?response_type=code&client_id={}&redirect_uri={}&scope={}&code_challenge={}&code_challenge_method=S256&state={}&id_token_add_organizations=true&codex_cli_simplified_flow=true&originator=clawde",
        CODEX_AUTHORIZE_URL,
        CODEX_CLIENT_ID,
...
pub async fn run_oauth_flow_with_label(
    ...
    clawde_core::oauth_config::save_codex_tokens_and_register(&tokens, label)?;
