...
///
/// This must stay **exactly aligned** with how [`crate::registry`] builds
/// each upstream's [`KeyRotatingProvider`] ring: `keys_for` only, no single
/// credential, and the OpenCode Zen/Go shared slots collapsed to a single
/// slot (zen first, go as fallback). The health poller probes this list by
/// index and forwards `key_idx` into the rings, so any divergence (a
/// prepended credential, merged slots) would mark the wrong key exhausted.
///
