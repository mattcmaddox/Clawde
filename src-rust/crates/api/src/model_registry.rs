...
//      `CLAWDE_MODELS_URL`) and writes it back to the on-disk cache.
//
...
impl ModelStatus {
    ...
    /// `CLAWDE_ENABLE_EXPERIMENTAL_MODELS=1`.
...
pub struct ModelRegistry {
    ...
    overrides: HashMap<String, clawde_core::config::ModelOverride>,
}
...
impl ModelRegistry {
    ...
    ///
    /// Set the `CLAWDE_ENABLE_EXPERIMENTAL_MODELS=1` env var to also include
    /// alpha/deprecated entries.
    pub fn list_visible_by_provider(&self, provider_id: &str) -> Vec<&ModelEntry> {
        let show_all = std::env::var("CLAWDE_ENABLE_EXPERIMENTAL_MODELS")
    ...
    /// was still fresh.  Honors `CLAWDE_DISABLE_MODELS_FETCH`.  All network
    ...
    ///
    /// [`ModelOverride`]: clawde_core::config::ModelOverride
    ...
    /// Register synthetic model entries for every upstream in the
    /// [`FREE_CATALOG`] so the model registry can resolve per-upstream
    /// metadata (tool_calling, context window, max output tokens) even
    /// for models that don't exist in the models.dev catalog.
    ///
    /// Each entry is keyed `"{upstream_id}/{default_model}"`, matching the
    /// lookup pattern in the query loop when a free upstream is dispatched
    /// directly (pin mode) or via the FreeProvider's own per-model override
    /// methods.
    ///
    /// [`FREE_CATALOG`]: crate::providers::free::FREE_CATALOG
    pub fn register_free_upstream_models(&mut self) {
        for upstream in crate::providers::free::FREE_CATALOG {
            let key = format!("{}/{}", upstream.id, upstream.default_model);
            // Don't overwrite a real catalog entry if one exists.
            if self.entries.contains_key(&key) {
                continue;
            }
            let entry = ModelEntry {
                info: ModelInfo {
                    id: ModelId::new(upstream.default_model),
                    provider_id: ProviderId::new(upstream.id),
                    name: format!("{} — {}", upstream.title, upstream.default_model),
                    context_window: 128_000,
                    max_output_tokens: upstream.max_tokens_cap.unwrap_or(8_192),
                    release_date: None,
                    status: None,
                },
                family: None,
...
mod tests {
    ...
    fn qwen_canonicalizes_to_alibaba_never_claude() {
        ...
        let cfg = clawde_core::config::Config {
    ...
    use clawde_core::config::ModelOverride;
