...
pub(crate) fn map_to_anthropic_event(
    evt: &clawde_api::StreamEvent,
) -> Option<clawde_api::AnthropicStreamEvent> {
    ...
    match evt {
        ...
        StreamEvent::MessageDelta { stop_reason, usage } => {
            ...
            let stop_reason_str = stop_reason.as_ref().map(|r| match r {
                clawde_api::provider_types::StopReason::ToolUse => "tool_use".to_string(),
                clawde_api::provider_types::StopReason::MaxTokens => "max_tokens".to_string(),
                clawde_api::provider_types::StopReason::StopSequence => "stop_sequence".to_string(),
                clawde_api::provider_types::StopReason::EndTurn => "end_turn".to_string(),
                clawde_api::provider_types::StopReason::ContentFiltered => {
                    "content_filtered".to_string()
                }
                clawde_api::provider_types::StopReason::Other(s) => s.clone(),
            });
