//! Rustail mascot rendering for ratatui.
//!
//! An 8-row Unicode block-art mascot. Call `rustail_lines()` to get
...
///
/// These durations are duplicated in `tools/logo-editor.html` (the Rustail
/// Animation Studio seed data); the `durations_match_rustail_studio` unit
...
/// Owned copy of the frames + durations, used by the in-TUI rustail editor.
/// Each entry is `(frame rows as 29-char strings, duration in ms)`.
pub(crate) fn rustail_frames_owned() -> Vec<(Vec<String>, u64)> {
    FRAMES
        .iter()
...
/// Returns 8 Lines representing the Rustail mascot.
///
...
pub fn rustail_lines(pose: &RustailPose) -> [Line<'static>; 8] {
    let idx = match pose {
        RustailPose::Default => 0,
        RustailPose::Loading { frame } => (*frame as usize) % FRAME_COUNT,
    };
...
mod tests {
    ...
    fn all_poses_produce_8_lines() {
        let poses = [RustailPose::Default, RustailPose::Loading { frame: 0 }];
        for pose in &poses {
            let lines = rustail_lines(pose);
    ...
    fn each_row_is_exactly_29_chars() {
        ...
        for frame_idx in 0..FRAME_COUNT {
            let lines = rustail_lines(&RustailPose::Loading {
    ...
    fn default_pose_row_0_matches_frame() {
        let lines = rustail_lines(&RustailPose::Default);
    ...
    fn loading_pose_cycles_through_frames() {
        ...
        for f in 0..(FRAMES.len() * 2) + 1 {
            let lines = rustail_lines(&RustailPose::Loading { frame: f as u64 });
    ...
    /// Parse the per-frame `dur:` values out of the Rustail Animation Studio
    ...
    #[test]
    fn durations_match_rustail_studio() {
        // The Rustail Animation Studio (tools/logo-editor.html) stores the
        ...
        for (i, (rust_ms, studio_ms)) in FRAME_DURATIONS_MS.iter().zip(&studio).enumerate() {
            assert_eq!(
                rust_ms, studio_ms,
                "frame {i} duration diverged from the Rustail Animation Studio \
