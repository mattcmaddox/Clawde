// rustle_editor.rs — In-TUI pixel editor for the Rustle mascot animation.
//
// Opened with `/rustle`. Edits the animation frames on their 8×29 grid with a
// keyboard glyph palette, then writes `FRAMES`, `FRAME_DURATIONS_MS` and
// `CYCLE_MS` back into `rustle.rs` so a rebuild picks up the changes.
//
...
impl EditableFrame {
    ...
    /// Rows back into the 29-char string form rustle.rs expects.
...
pub enum RustleEditAction {
    /// The frames were written back to rustle.rs.
    Saved,
}

pub struct RustleEditor {
    pub visible: bool,
...
impl RustleEditor {
    pub fn new() -> Self {
        Self {
            visible: false,
            frames: rustle::rustle_frames_owned()
                .into_iter()
    ...
    /// Write the edited frames back into rustle.rs AND the preset store.
    pub fn save(&mut self) -> Result<(), String> {
        ...
        // Then write consts into rustle.rs.
        let path = rustle_source_path();
...
impl Default for RustleEditor {
    fn default() -> Self {
        Self::new()
    }
}

/// Load frames from the preset store, falling back to rustle.rs built-ins.
...
// ---------------------------------------------------------------------------
// rustle.rs const regeneration (pure + unit-testable)
// ---------------------------------------------------------------------------

/// The path to the crate's own `rustle.rs` — the editor rewrites its source.
fn rustle_source_path() -> PathBuf {
...
/// Replace the three consts inside the current rustle.rs source text.
fn replace_consts(
    source: &str,
    frames_text: &str,
    dur_line: &str,
    ...
    let fr_start = source
        .find("const FRAMES:")
        .ok_or_else(|| "could not find the FRAMES const in rustle.rs".to_string())?;
    ...
    let dur_start = source
        ...
        .ok_or_else(|| "could not find FRAME_DURATIONS_MS in rustle.rs".to_string())?;
    ...
    let cyc_start = source
        ...
        .ok_or_else(|| "could not find the CYCLE_MS const in rustle.rs".to_string())?;
    ...
    // rustle.rs.  All three markers are structural (`const NAME:` and
...
/// Returns `Some(RustleEditAction)` when the editor wants the app to react.
pub fn handle_rustle_editor_key(
    screen: &mut RustleEditor,
    key: KeyEvent,
) -> Option<RustleEditAction> {
    if !screen.visible {
        return None;
    }
...
pub fn render_rustle_editor(frame: &mut Frame, screen: &RustleEditor, area: Rect) {
    if !screen.visible {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn editor_loads_frames_from_rustle() {
        let editor = RustleEditor::new();
    ...
    fn paint_erase_and_undo() {
        let mut editor = RustleEditor::new();
        editor.visible = true;
        // Move to (1,1) then paint the current glyph.
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char('j')));
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char('l')));
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char(' ')));
        let painted = editor.frames[0].rows[1][1];
        assert_eq!(painted, GLYPHS[0]);
        assert!(editor.dirty);

        // Undo restores the original cell.
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char('u')));
        assert_eq!(editor.frames[0].rows[1][1], ' ');

        // Erase works too.
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char(' ')));
        handle_rustle_editor_key(&mut editor, key(KeyCode::Backspace));
        assert_eq!(editor.frames[0].rows[1][1], ' ');
    }
    ...
    fn glyph_keys_select_palette() {
        ...
        for (i, letter) in GLYPH_KEYS.iter().enumerate() {
            handle_rustle_editor_key(&mut editor, key(KeyCode::Char(*letter)));
    ...
    fn digits_goto_frame_and_set_duration() {
        ...
        editor.visible = true;
        // '3' → frame index 2.
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char('3')));
    ...
    fn cannot_remove_last_frame() {
        ...
        for _ in 0..5 {
            handle_rustle_editor_key(&mut editor, key(KeyCode::Char('-'))); // warn
            handle_rustle_editor_key(&mut editor, key(KeyCode::Char('-'))); // remove
        }
    ...
    fn duplicate_frame_copies_grid_and_duration() {
        ...
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char('v'))); // pick ▚ glyph
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char(' '))); // paint at (0,0)
        editor.frames[0].dur_ms = 4200;

        let orig_len = editor.frames.len();
        let ctrl_d = KeyEvent::new(KeyCode::Char('d'), KeyModifiers::CONTROL);
        handle_rustle_editor_key(&mut editor, ctrl_d);
        ...
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char('d')));
    ...
    fn esc_requires_confirmation_when_dirty() {
        let mut editor = RustleEditor::new();
        editor.visible = true;
        handle_rustle_editor_key(&mut editor, key(KeyCode::Char(' '))); // make dirty
        assert!(editor.visible);
        handle_rustle_editor_key(&mut editor, key(KeyCode::Esc));
...
";
    ...
    fn replace_consts_does_not_double_closing_semicolon() {
        // Source shaped like the real rustle.rs: frames const immediately
...
";
    ...
    fn render_editor(screen: &RustleEditor) -> String {
        let backend = TestBackend::new(100, 30);
        let mut terminal = Terminal::new(backend).unwrap();
        terminal
            .draw(|frame| render_rustle_editor(frame, screen, frame.area()))
            .unwrap();
    ...
    fn editor_renders_baseline_ui() {
        let mut screen = RustleEditor::new();
        screen.open();
    ...
    fn editor_renders_confirm_on_non_blank_frame() {
        ...
        handle_rustle_editor_key(&mut screen, key(KeyCode::Char('-')));
