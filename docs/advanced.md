...
This document covers Clawde's advanced capabilities beyond basic coding assistance.
...
```
clawde --thinking <tokens>    Set a specific token budget for thinking
clawde --effort <level>       Set the effort level (low/medium/high/max)
```
...
**Environment variable override:** `CLAUDE_CODE_EFFORT_LEVEL` overrides the persisted setting for the current process. If this variable is set and conflicts with a `/effort` command, Clawde displays a warning.
...
Clawde tracks token usage after every model turn. When usage crosses the auto-compact threshold — which is the effective context window size minus a 13,000-token buffer — it runs `compactConversation` to summarise the history and replaces the messages with a compact summary plus any trailing context.
...
```bash
DISABLE_COMPACT=1 clawde
```

**Override the threshold window (for testing):**

```bash
CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=80 clawde
```
...
Sessions are stored as JSONL files under `~/.clawde/projects/<base64url(project-root)>/<session-id>.jsonl`. Each line in the file is a JSON object representing a message or event in the conversation. The per-session metadata index lives at `~/.clawde/sessions/<id>.json`.
...
The `parentUuid` field forms a linked chain that allows Clawde to reconstruct the conversation tree. `/fork` rewrites all UUIDs while preserving the chain structure.
...
The goal system lets Clawde work autonomously across multiple turns toward a single, verifiable objective. Instead of prompting repeatedly, you set the goal once and Clawde iterates until the goal is complete, paused, or the built-in runaway guard fires.
...
Once a goal is set, Clawde begins working immediately. It continues across turns without waiting for user input until one of these conditions is met:
...
Clawde displays both to the user before marking the goal complete. The model is expected to genuinely audit the outcome before calling; calling without real evidence is rejected.
...
Configuration persists to `~/.clawde/settings.json` under `managed_agents`.
...
Headless mode runs Clawde non-interactively, suitable for scripts, CI pipelines, and programmatic orchestration.

### --print flag

```bash
clawde --print "refactor this function to use async/await"
clawde -p "summarise the changes in this PR"
```
...
```bash
cat my_prompt.txt | clawde --print
echo "explain this code" | clawde -p
```
...
```bash
clawde --max-budget-usd 2.00 "..."   # Stop after spending $2.00
clawde --max-turns 10 "..."          # Stop after 10 model turns
clawde --max-tokens 50000 "..."      # Stop after 50,000 output tokens
```

When a limit is reached, Clawde exits with a corresponding error message:
...
Every Clawde user gets a persistent companion derived deterministically from their user ID. The companion appears as a small sprite in the terminal UI and occasionally comments on activity.
...
Clawde can be controlled remotely through a web interface at claude.ai. This "bridge" mode keeps a WebSocket connection open that allows a remote UI to send prompts and receive streaming responses.
...
- **In-process bridge** — the WebSocket lives inside the Clawde process. If the process dies, the connection is lost.
...
SSH sessions work similarly: `clawde --ssh` enables a remote-accessible session that can be connected to from another machine.

---

## AGENTS.md hierarchical memory

Clawde reads instruction files from the filesystem before every session and whenever a relevant file changes. The lookup order is:

1. **Managed** — `/etc/claude-code/AGENTS.md` and `/etc/claude-code/CLAUDE.md` (administrator-controlled, always loaded).
2. **User** — `~/.clawde/AGENTS.md` then `~/.clawde/CLAUDE.md` (personal global instructions).
3. **Project** — `AGENTS.md`, `CLAUDE.md`, `.clawde/AGENTS.md`, and `.clawde/CLAUDE.md` in each directory from the filesystem root down to the current working directory. Files are loaded from the root toward the CWD so that parent-directory rules are visible when processing child-directory rules.
...
```bash
clawde --add-dir /path/to/additional/project "..."
```

Grants Clawde read access to an additional directory outside the working directory. Useful when a task spans multiple repositories or when config files live outside the project root.
